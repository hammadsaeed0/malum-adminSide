...
const fetchCustomer = () => {
  ...
  fetch(
    ...
    .then(result => {
      ...
      setToCustomer(data.data.customers)
...
const getDataCustomer = () => {
  ...
  console.log('second selected data---', toselected);
  const TocustomerID = tocustomer.filter(i => {
    return i.name === toselected;
  });
  ...
  // console.log(TocustomerID[0].uid);
  var formdata = new FormData();
formdata.append("__api_key__", "hi,-its-eevee. I can do wonderful things in api creation.");
formdata.append("customer_one_uid", customerID[0].uid);
formdata.append("customer_uid", TocustomerID[0].uid);
...
fetch("https://api.accounts.lighthousepakistan.online/add_customer_payment.php", requestOptions)
  .then(response => response.text())
  .then(result => {
    const data = JSON.parse(result);
    if(data.state === "OK"){
    ...
    <View style={{flex:1, backgroundColor:Color.white}}>
      ...
      <ScrollView contentContainerStyle={{width:responsiveWidth(90) , height:responsiveHeight(100), alignSelf:'center', marginTop:responsiveHeight(0)}}>
        ...
        setSelected={val => setToSelected(val)}
          data={
            tocustomer
              ? tocustomer?.map(i => ((value = i.name), (key = i.name)))
              : data
          }
        />
