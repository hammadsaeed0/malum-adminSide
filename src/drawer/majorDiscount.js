...
const MajorDiscount = ({navigation}) => {
...
const [payment , setPayment] = useState([])
  const fetchDirectPayment = () =>{
    ...
    fetch("https://api.accounts.lighthousepakistan.online/fetch_major_discounts.php", requestOptions)
      .then(response => response.text())
      .then(result => {
        const data = JSON.parse(result);
        console.log(data.data);
        setPayment(data.data.major_discounts)
      })
...
{/* // Customer Card */}
      ...
      <View style={{width:responsiveWidth(90), height:responsiveHeight(10), backgroundColor:'white', alignSelf:'center', flexDirection:'row', justifyContent:'space-between', alignItems:'center', borderRadius:10, elevation:3}}>
        <Text style={{fontFamily:Font.Regular, color:Color.primery, fontSize:responsiveFontSize(3), marginLeft:responsiveWidth(3)}}>Major Discount</Text>
        <TouchableOpacity style={{alignItems:'center', justifyContent:"center", width:60, height:30, backgroundColor:Color.primery, borderRadius:5, marginRight:responsiveWidth(3)}} onPress={()=> navigation.navigate('AddMajorDiscount')}>
